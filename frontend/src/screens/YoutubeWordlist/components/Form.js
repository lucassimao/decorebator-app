...
function Form(props) {
  ...
  const { loading, error, data } = useQuery(YOUTUBE_SUBTITLES_QUERY, {
    variables: { url },
  });
    ...
    <>
      <Grid item xs={12}>
        <FormControl>
          ...
          >
            {data.subtitles
              .filter(({ isAutomatic }) => !isAutomatic || allowAsr)
              .map((subtitle, index) => (
        ...
        <FormControlLabel
          control={
            <Switch
              ...
              onChange={(evt) =>
                dispatch({
                  type: "SET_ONLY_NEW_WORDS",
                  onlyNewWords: evt.target.checked,
                })
              }
            />
          }
